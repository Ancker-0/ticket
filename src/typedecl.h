...
const auto trainID_checker = ([](std::string s) {
  return (((((s).length()) <= (20))) && (({
            ...
            for (auto &tmpvar36 : (s))
              if (not(({
                    bool flag37 = false;
                    for (auto &tmpvar38 : ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi"
                                           "jklmnopqrstuvwxyz0123456789_"))
...
struct train_t {
  ...
  train_t(std::string s0, std::string s1, std::string s2, std::string s3,
    ...
    assert(tmpvar67.size() == (stationNum));
