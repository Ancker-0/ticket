...
Trainer::Trainer(): bf("trainer.db"), bf2("trainer-vec.db"), vec_ass(bf2), db(bf) {
}
...
void Trainer::refund_ticket(trainID_t ID, date_t date, stationName_t from, stationName_t to, seatNum_t num) {
  ...
  for (int i = 0; i < blah.size(); ++i) {
    ...
    if (blah[i].status == 0) {
      ...
      bool success = true;
      for (int j = ps; j < pt; ++j)
        if (getSeat(train.seat[blah[i].realdate][j]) + blah[i].num > train.seatNum) {
          success = false;
          break;
        }
      if (success) {
...
sjtu::vector<Trainer::qry_ticket_t> Trainer::query_ticket(date_t date, stationName_t start, stationName_t end,
  ...
  auto asker = [&](const train_t &train) {
    ...
    if (after == invalid_time) {
      ...
      if (time_and_date_less({realdate + leaving_time.first, leaving_time.second}, {date, after}))
        ++realdate;
      if (realdate > train.saleDate[1])
        return;
      assert(!time_and_date_less({realdate + leaving_time.first, leaving_time.second}, {date, after}));
    }
  ...
  */
  if (ret.empty())
    return ret;
  if (sort_by_cost)
    sjtu::sort(&ret[0], &ret[0] + ret.size(), [&](const auto &u, const auto &v) { return u.price == v.price ? u.trainID < v.trainID : u.price < v.price; });
  else
    sjtu::sort(&ret[0], &ret[0] + ret.size(), [&](const auto &u, const auto &v) {
        if (time_and_date_diff(u.leaving_time, u.arriving_time) == time_and_date_diff(v.leaving_time, v.arriving_time))
          return u.trainID < v.trainID;
        return time_and_date_diff(u.leaving_time, u.arriving_time) < time_and_date_diff(v.leaving_time, v.arriving_time);
      });
  return ret;
}
...
Trainer::transfer_t Trainer::query_transfer(date_t date, stationName_t start, stationName_t end,
  ...
  auto asker = [&](const train_t &train) {
    ...
    for (int i = ps + 1; i < train.stationNum; ++i) {
      ...
      arriving_time.first += realdate;
      sjtu::vector<qry_ticket_t> vec = query_ticket(arriving_time.first, train.stationNames[i], end, sort_by_cost, arriving_time.second);
      if (vec.empty())
        continue;
      sjtu::sort(&vec[0], &vec[0] + vec.size(), [&](auto u, auto v) {
        if (u.trainID == train.trainID)
          return false;
        if (v.trainID == train.trainID)
          return true;

        @(define-syntax compare-by
           (syntax-rules ()
             [(_ [u v])
              (return ((op "<") u v))]
             [(_ [u v] [us vs] ...)
              (COND
                [(== u v) (compare-by [us vs] ...)]
                [else (return ((op "<") u v))])]))
        if (sort_by_cost) {
          @(compare-by ["u.price" "v.price"]
                       ["u.arriving_time" "v.arriving_time"]
                       ["u.trainID" "v.trainID"]);
        } else {
          @(compare-by ["u.arriving_time" "v.arriving_time"]
                       ["u.price" "v.price"]
                       ["u.trainID" "v.trainID"]);
        }
      });
      if (vec[0].trainID != train.trainID) {
        ret.push_back(transfer_t{qry_ticket_t{train.trainID, leaving_time, arriving_time, price, train.seatNum - seat}, vec[0], train.stationNames[i]});
      }
    }
  };
  ...
  sjtu::sort(&ret[0], &ret[0] + ret.size(), [&](auto u, auto v) {
    ...
    if (sort_by_cost) {
      @(compare-by ["priceu" "pricev"]
                   ["timeu" "timev"]
                   ["u.q1.trainID" "v.q1.trainID"]
                   ["u.q2.trainID" "v.q2.trainID"])
    } else {
      @(compare-by ["timeu" "timev"]
                   ["priceu" "pricev"]
                   ["u.q1.trainID" "v.q1.trainID"]
                   ["u.q2.trainID" "v.q2.trainID"])
    }
